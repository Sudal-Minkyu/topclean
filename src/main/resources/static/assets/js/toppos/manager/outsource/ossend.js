...
const dtos = {
    send: {
        branchReceiptBranchInOutsouringList: {
...
const comms = {
    getFrList() {
        CommonUI.ajax('/api/manager/branchBelongList', "GET", false, function (res) {
            ...
            dv.chk(data, dtos.receive.managerBelongList, "가맹점 선택 외주출고에 필요한 지점에 속한 가맹점 받아오기");
            const $frList = $("#frList");
            data.forEach(obj => {
    ...
    getCandidateList(searchCondition) {
        dv.chk(searchCondition, dtos.send.branchReceiptBranchInOutsouringList, '외주 품목 조회 조건 보내기');
        CommonUI.ajax('/api/manager/branchReceiptBranchInOutsouringList', "GET", searchCondition, function (res) {
    ...
    sendOutsource(sendList) {
        wares.sl = sendList;
        dv.chk(sendList, dtos.send.branchStateChange, "외주출고처리 항목 보내기");
        CommonUI.ajax('/api/manager/branchStateOutsouringChange', "PARAM", sendList, function (res) {
            const data = res.sendData;
            dv.chk(data, dtos.receive.branchStateChange, "외주출고처리된 항목 외주출고증 인쇄 위한 miNoList 받아오기");
            alertSuccess("외주출고처리가 완료 되었습니다.");
            grids.f.clearData(0);
            grids.f.clearData(1);
            wares.candidateList = "";
    ...
    //     dv.chk(miNoList, dtos.send.branchDispatchPrint, "외주출고증 인쇄를 위한 miNoList 보내기");
...
const grids = {
    ...
    f: { // 그리드 펑션
        ...
        exportToXlsx() {
            ...
            const isOutsourceable = wares.currentDetail.isOutsourceable ? wares.currentDetail.isOutsourceable + "_" : "";

            AUIGrid.exportToXlsx(grids.s.id[0], {
                fileName : `${wares.title}_${frName}${isOutsourceable}${wares.currentDetail.filterFromDt}_${wares.currentDetail.filterToDt}`,
                progressBar : true,
            });
        },
    ...
    basic() {
        ...
        $("#sendOutBtn").on("click", function () {
            ...
            if(wares.checkedItems.length) {
                alertCheck("선택된 상품을 외주출고처리 하시겠습니까?");
                $("#checkDelSuccessBtn").on("click", function () {
                    sendOut();
                    $('#popupId').remove();
                });
            } else {
                alertCaution("외주출고처리할 상품을 선택해 주세요.", 1);
            }
        });
...
const wares = {
    candidateList: "",
    checkedItems: [],
    title: "외주출고", // 엑셀 다운로드 파일명 생성에 쓰인다.
...
function inputTag() {
    ...
    for(const item of gridItems) {
        if (item.fdTag === tagNo) {
            ...
            listCheckChanged();
            $("#inputTagNo").val("");
            if (item.bpOutsourcingYn !== "Y") {
                notAvailableWarn(item);
                return;
            }
...
function sendOut() {
    ...
    if(wares.checkedItems.length) {
        ...
        comms.sendOutsource(sendList);
    } else {
        alertCaution("외주출고할 품목을 선택해 주세요.", 1);
    }
}
...
function notAvailableWarn(item) {
    grids.f.addUncheckedRowByTagNo(item.fdTag);
    listCheckChanged();
    alertCaution(`해당 택번호(${CommonData.formatBrTagNo(item.fdTag)})는<br>외주 처리 대상 품목이 아닙니다.`
        + `<br>외주관리 > 외주관리가격설정을 통해<br>외주 처리 대상 설정 할 수 있습니다.`, 1);
    CommonUI.toppos.speak("외주출고처리 할 수 없는 상품입니다.");
}
