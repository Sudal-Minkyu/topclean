...
const comms = {
    ...
    getMainGridList(searchCondition) {
        dv.chk(searchCondition, dtos.send.branchReceiptReturnReleaseList, "메인 그리드 검색 조건 보내기");
        CommonUI.ajax(urls.getMainGridList, "GET", searchCondition, function (res) {
            ...
            dv.chk(data, dtos.receive.branchReceiptReturnReleaseList, "메인 그리드 검색 결과 받기");
...
const wares = {
    checkedItems: [],
    title: "가맹점반품출고", // 엑셀 다운로드 파일명 생성에 쓰인다.
...
function askExcute() {
    ...
    if(wares.checkedItems.length) {
        alertCheck("선택된 상품을 반품출고처리 하시겠습니까?");
        $("#checkDelSuccessBtn").on("click", function () {
            executeCheckedReceipts();
        });
    } else {
        alertCaution("반품출고처리할 품목을 선택해 주세요.", 1);
    }
}
