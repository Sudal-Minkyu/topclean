...
class CATPayment {
     ...
     * @constructor
     */
    CatPrint_Release(releaseInfo) {
        let message="";
        message = CatCreate_ReleasePrint(releaseInfo);
    ...
    CatCancelCashReceipt(params, func) {
    ...
    CatPrint_Repayment(params, creditData, cancelYN) {
        let message="";
        message = CatCreate_RepaymentPrint(params,creditData,cancelYN)
...
function CatCreate_StatementOfAccounts(params) {
    ...
    message += String.fromCharCode(10);
    message += "잔여  미수금 :".fillSpaceUnicode(32) + (numberWithCommasCat(params.c9)).padStart(10);
    message += String.fromCharCode(10);
    message += String.fromCharCode(10);
...
function CatCreate_PrintTest(){
    ...
    // message += String.fromCharCode(10);
    // /* 영업시간을 구성하는 부분 */
    // if (params.isPrintBusinessTimeOn) {
    //     message += "영업시간   ";
    //     let isNotFirstBusinessTimePrintRow = false;
    //     const korDayTypeEnum = {Weekday: "평  일", Saturday: "토요일", Holiday: "공휴일"};
    //     for (const dayType of ["Weekday", "Saturday", "Holiday"]) {
    //         if (["1", "2", "3"].includes(params["frStat" + dayType])) {
    //             if (isNotFirstBusinessTimePrintRow) {
    //                 message += "           " + korDayTypeEnum[dayType] + "  ";
    //             } else {
    //                 message += korDayTypeEnum[dayType] + "  ";
    //                 isNotFirstBusinessTimePrintRow = true;
    //             }
    //         }
    //         switch (params["frStat" + dayType]) {
    //             case "1":
    //                 message += params["frOpen" + dayType] + " ~ " + params["frClose" + dayType] + " 까지";
    //                 message += String.fromCharCode(10);
    //                 break;
    //             case "2":
    //                 message += "24시간 오픈";
...
function CatCreate_ReleasePrint(params)  {
    let message = "";
    ...
    message += String.fromCharCode(33);
    message += String.fromCharCode(8);
    message += params.releaseDateTime.padStart(18);
    message += String.fromCharCode(27);
    message += String.fromCharCode(33);
    message += String.fromCharCode(0);
    message += String.fromCharCode(10);
...
function CatCreate_RepaymentPrint(params,creditResults,cancelYN) {
    ...
    creditResults.forEach((creditResult,idx)=>{
        ...
        message += String.fromCharCode(10);
        message += "미수금잔액:".fillSpaceUnicode(33) + (" " + numberWithCommasCat(params.totalUncollectAmount)).padStart(9);
        message += String.fromCharCode(10);
        if (creditResult.type ==='card') {
            let month = "일시불";
            if ((creditResult.month) > 1 ){
...
function CatCashCancel_vPOSV1(params) {
    ...
    // 전문길이 마지막에 입력
    request_msg += "d1";                                       // 전문구분코드
    request_msg += "00";                                       // 할부개월
    request_msg += fcType;                                     	 // 현금영수증 소비자 구분 (1: 소비자 소득공제, 2: 사업자 지출증빙, 3: 자진발급)
    request_msg += "1";                                        // 현금영수증 취소 사유 (1: 거래 취소)
    request_msg += totalAmount;                                // 총금액
    request_msg += vatAmount;                                // 세금
    request_msg += "000000000";                             	 // 면세금액
    request_msg += "000000000";                                // 봉사료
    request_msg += approvalTime;                                // 원거래 일시
    request_msg += approvalNo;                                  // 원거래 승인번호
    request_msg += franchiseNo;                                     // 거래일련번호
    request_msg += String.fromCharCode( 3);                    // ETX
