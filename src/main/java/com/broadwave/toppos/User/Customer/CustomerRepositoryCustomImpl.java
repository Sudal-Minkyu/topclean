...
public class CustomerRepositoryCustomImpl extends QuerydslRepositorySupport implements CustomerRepositoryCustom {
    ...
    // 고객 핸드폰 번호 중복체크용
    @Override
    public List<CustomerHpCheckDto> findBybcHpCheck(String frCode, String bcHp) {

        QCustomer customer = QCustomer.customer;

        JPQLQuery<CustomerHpCheckDto> query = from(customer)
                .where(customer.frCode.eq(frCode).and(customer.bcHp.eq(bcHp)))
                .select(Projections.constructor(CustomerHpCheckDto.class,
                        customer.bcId,
                        customer.bcName,
                        customer.bcHp
                ));
