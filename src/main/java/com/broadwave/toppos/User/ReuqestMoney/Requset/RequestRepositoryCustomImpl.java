...
public class RequestRepositoryCustomImpl extends QuerydslRepositorySupport implements RequestRepositoryCustom {
    ...
    public RequestRepositoryCustomImpl(JdbcTemplate jdbcTemplate) {
        super(Request.class);
...
//        }
    ...
    // 문자 메세지 테이블 Native쿼리 -> messageType : "L" LMS(유료문자), "S"(90문자이하)  (사용하지않음)
    @Override
    ...
    // 문자 메세지 테이블  jdbcTemplate.batchUpdate 활용
    @Transactional
    @Override
    public boolean smsMessageBatchInsert(String message, List<String> bcHpList, String tableName, List<MessageHistory> saveMessageHistorieList,
                            String templatecodeNumber, String msgType, LocalDateTime sendreqTime) {
        String sqlQueryString = "INSERT INTO SMSQ_SEND (`dest_no`, `call_back`, `msg_contents`, `msg_type`, `app_etc1`, `app_etc2`, `msg_instm`, `sendreq_time`) VALUES (?, ?, ?, ?, ?, ?, NOW(),";
        if(sendreqTime != null){
            sqlQueryString += "?)";
        }else{
            sqlQueryString += "NOW())";
        ...
        jdbcTemplate.batchUpdate( sqlQueryString, new BatchPreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                ps.setString(1, bcHpList.get(i));
                ps.setString(2, templatecodeNumber);
                ps.setString(3, message);
                ps.setString(4, msgType);
                ps.setString(5, tableName);
                ps.setLong(6, saveMessageHistorieList.get(i).getFmId());
                if(sendreqTime != null){
                    ps.setTimestamp(7, Timestamp.valueOf(sendreqTime));
                }
            }
            @Override
            public int getBatchSize() {
                return saveMessageHistorieList.size();
            }
        });
...
//                                                .otherwise(inspeot2.fiProgressStateDt))
    ...
    // 본사 세탁상태 접수 왼쪽 NativeQuery
    @Override
    public List<RequestUrgentReceiptListDto> findByHeadUrgentReceiptList(Long branchId, Long franchiseId, String filterFromDt, String filterToDt, String fdUrgentYn, String fdUrgentType) {
        EntityManager em = getEntityManager();
        StringBuilder sb = new StringBuilder();
        ...
        sb.append("AND a.fr_yyyymmdd>= ?1 \n");
        sb.append("AND a.fr_yyyymmdd<= ?2 \n");
        sb.append("AND b.fd_urgent_yn= ?3 \n");
        sb.append("AND b.fd_urgent_type= ?4 \n");
        if(branchId != 0){
            sb.append("AND d.br_id = ?5 \n");
            if(franchiseId != 0){
                sb.append("AND c.fr_id = ?6 \n");
            }
            sb.append("GROUP BY d.br_name, c.fr_name, a.fr_yyyymmdd ORDER BY d.br_name, c.fr_name, a.fr_yyyymmdd ASC; \n");
        }else{
            sb.append("GROUP BY d.br_name, a.fr_yyyymmdd ORDER BY d.br_name, a.fr_yyyymmdd ASC; \n");
        ...
        query.setParameter(1, filterFromDt);
        query.setParameter(2, filterToDt);
        query.setParameter(3, fdUrgentYn);
        query.setParameter(4, fdUrgentType);
        if(branchId != 0){
            query.setParameter(5, branchId);
            if(franchiseId != 0){
                query.setParameter(6, franchiseId);
            }
        }

        return jpaResultMapper.list(query, RequestUrgentReceiptListDto.class);
    }

    // 본사 세탁상태 접수 오른쪽 - querydsl
    public List<RequestUrgentReceiptListSubDto> findByHeadUrgentReceiptSubList(Long branchId, Long franchiseId, String frYyyymmdd, String fdUrgentYn, String fdUrgentType) {
        ...
        JPQLQuery<RequestUrgentReceiptListSubDto> query = from(requestDetail)
                .innerJoin(requestDetail.frId, request)
                .innerJoin(request.bcId, customer)
                .innerJoin(franchise).on(franchise.frCode.eq(request.frCode))
                .innerJoin(branch).on(branch.brCode.eq(franchise.brCode))
                ...
                .where(request.frConfirmYn.eq("Y"))
                .where(requestDetail.fdCancel.eq("N").and(requestDetail.fdUrgentYn.eq(fdUrgentYn).and(requestDetail.fdUrgentType.eq(fdUrgentType))))

                .select(Projections.constructor(RequestUrgentReceiptListSubDto.class,
    ...
    // 본사 반품현황 왼쪽 NativeQuery
    @Override
    public List<RequestReturnReceiptListDto> findByHeadReturnReceiptList(Long branchId, Long franchiseId, String filterFromDt, String filterToDt) {
        EntityManager em = getEntityManager();
        StringBuilder sb = new StringBuilder();

        sb.append("SELECT \n");
        sb.append("d.br_id, d.br_name, c.fr_id, c.fr_name, b.fd_s6_dt, COUNT(*), SUM(b.fd_tot_amt) \n");
        sb.append("FROM  fs_request a \n");
        sb.append("INNER JOIN fs_request_dtl b on b.fr_id=a.fr_id \n");
        sb.append("INNER JOIN bs_franchise c on c.fr_code=a.fr_code \n");
        sb.append("INNER JOIN bs_branch d on d.br_code = c.br_code \n");
        ...
        if(branchId != 0){
            ...
            sb.append("GROUP BY d.br_name, c.fr_name, b.fd_s6_dt ORDER BY d.br_name, c.fr_name, b.fd_s6_dt ASC; \n");
        }else{
            sb.append("GROUP BY d.br_name, b.fd_s6_dt ORDER BY d.br_name, b.fd_s6_dt ASC; \n");
        }
        ...
        return jpaResultMapper.list(query, RequestReturnReceiptListDto.class);
    }

    //  본사 접수현황 오른쪽 - querydsl
    public List<RequestReturnReceiptListSubDto> findByHeadReturnReceiptSubList(Long branchId, Long franchiseId, String fdS6Dt) {
        ...
        JPQLQuery<RequestReturnReceiptListSubDto> query = from(requestDetail)
                .innerJoin(requestDetail.frId, request)
                .innerJoin(request.bcId, customer)
                .innerJoin(franchise).on(franchise.frCode.eq(request.frCode))
                .innerJoin(branch).on(branch.brCode.eq(franchise.brCode))
                ...
                .select(Projections.constructor(RequestReturnReceiptListSubDto.class,
    ...
    //  본사 택번호조회 - querydsl
    public List<RequestTagNoReceiptListDto> findByHeadTagNoReceiptSubList(Long branchId, Long franchiseId, String tagNo) {
        ...
        JPQLQuery<RequestTagNoReceiptListDto> query = from(requestDetail)
                .innerJoin(requestDetail.frId, request)
                .innerJoin(request.bcId, customer)
                .innerJoin(franchise).on(franchise.frCode.eq(request.frCode))
                .innerJoin(branch).on(branch.brCode.eq(franchise.brCode))
                ...
                .select(Projections.constructor(RequestTagNoReceiptListDto.class,
